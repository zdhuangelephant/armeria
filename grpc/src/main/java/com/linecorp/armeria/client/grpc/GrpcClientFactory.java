 ...
 * <br/>
 * 创建GRPCClient的ClientFactory
 */
final class GrpcClientFactory extends DecoratingClientFactory {

    // SessionProtocol.values()集合1 和 GrpcSerializationFormats.values()集合2  求笛卡尔积
     ...
     * <br/>
     * 返回一个支持"none+http"的ClientFactory实例
    ...
    public static void main(String[] args) {
       Set<Scheme> SUPPORTED_SCHEMES =
                Arrays.stream(SessionProtocol.values())
                        .flatMap(p -> GrpcSerializationFormats.values()
                                .stream()
                                .map(f -> Scheme.of(f, p)))
                        .collect(toImmutableSet());
       SUPPORTED_SCHEMES.stream().forEach(item -> System.out.println(item));
    }
