 ...
 * <br/>
 * 表示一个主机的时候 eg: {@code "<host>"} or {@code "<host>:<port>"}. 这个时候即不需要去走DNS
 *
 *
 ...
 * <br/>
 * 如果是多个主机构成的组，那么就会需要{@link #resolve(ClientRequestContext)}从组内挑选其中一个的Endpoint(挑选的策略，包括一致性哈希、轮询、权重配比)
 */
public final class Endpoint implements Comparable<Endpoint> {
     ...
     * <br/>
     * 通过EndpointGroupRegistry传入的参数ClientRequestContext内的Endpoint组，选中其中的一个Endpoint。
     *
