...
/**
 * 构建一个新的Server并且与之绑定一个ServerConfig
 * <br/>
 *
 ...
 *
 * <p>如果我们有指定明确的端口，则会随机选择一个不用的端口来给我们默认绑定</p>
...
public final class ServerBuilder {
    ...
    private static final String DEFAULT_SERVICE_LOGGER_PREFIX = "armeria.services"; // 默认的Service日志前缀
    private static final int PROXY_PROTOCOL_DEFAULT_MAX_TLV_SIZE = 65535 - 216;     //
    private static final String DEFAULT_ACCESS_LOGGER_PREFIX = "com.linecorp.armeria.logging.access"; // 默认的访问日志前缀

    // Prohibit deprecate option  禁用socket选项
    ...
    private final List<ServerPort> ports = new ArrayList<>();   // 当前Server监听的端口集合
    private final List<ServerListener> serverListeners = new ArrayList<>(); // 监听此erver的所有监听者们的容器
    private final List<VirtualHostBuilder> virtualHostBuilders = new ArrayList<>();  // 虚拟主机的集合
    private final VirtualHostBuilder defaultVirtualHostBuilder = new VirtualHostBuilder(this, true); //客户端调用service()的所有的path声明会全部存入这个defaultVirtualHostBuilder。 将测ServerBuilder传递给VirtualHostBuilder。并声明一个默认的VirtualHostBuilder。

    private EventLoopGroup workerGroup = CommonPools.workerGroup();  // 线程池组。其内部管理着多个线程池。
    private boolean shutdownWorkerGroupOnStop;   // 在Server停止的时候，是否关闭worker线程。
    private Executor startStopExecutor = GlobalEventExecutor.INSTANCE;   // 专门负责启动关闭线程的另外一个线程池[内部始终只有一个线程在运行]。
    private final Map<ChannelOption<?>, Object> channelOptions = new Object2ObjectArrayMap<>();  // channel的配置选项。
    private final Map<ChannelOption<?>, Object> childChannelOptions = new Object2ObjectArrayMap<>(); //
    private int maxNumConnections = Flags.maxNumConnections();   // Server允许的最大连接数
    private long idleTimeoutMillis = Flags.defaultServerIdleTimeoutMillis();    // 空闲超时时间
    private int http2InitialConnectionWindowSize = Flags.defaultHttp2InitialConnectionWindowSize();     // http2新协议的链接窗口大小
    private int http2InitialStreamWindowSize = Flags.defaultHttp2InitialStreamWindowSize();     // http2新协议流窗口大小
    private long http2MaxStreamsPerConnection = Flags.defaultHttp2MaxStreamsPerConnection();    // http2新协议 每个链接最大的流
    private int http2MaxFrameSize = Flags.defaultHttp2MaxFrameSize();       // http2新协议 最大的帧大小
    private long http2MaxHeaderListSize = Flags.defaultHttp2MaxHeaderListSize();    // http2新协议 可允许最大的Header的数量
    ...
    private Executor blockingTaskExecutor = CommonPools.blockingTaskExecutor();    //  阻塞任务执行的线程池
    ...
    // These properties can also be set in the service level.  下面的这些属性在Service级别也可以被覆盖
    ...
    @Nullable
    private Function<Service<HttpRequest, HttpResponse>, Service<HttpRequest, HttpResponse>> decorator;  // 装饰者将Service转化为另一个Service
     ...
     * 可以让多个协议在同一个端口进行监听。这里通过使用代理协议来进行支持
     * <pre>{@code
     * ServerBuilder sb = new ServerBuilder();
     * // Serve both HTTP and HTTPS at port 8080.  在8080端口同时进行http和https的协议支持
     * sb.port(8080,
     ...
     *
     * <br/>
     * 1、workerGroup是处理Socket I/O，即 处理所有的业务请求
     * 2、如果没有设置的话，则默认使用{@linkplain CommonPools#workerGroup()
     *
     * @param shutdownOnStop whether to shut down the worker {@link EventLoopGroup}
     *                       when the {@link Server} stops 当Server关闭的时候， 是否关闭workerGroup
     */
     ...
     * <br/>
     * 执行阻塞任务的线程池；
     * 如果用户没有显示的设置，则默认会使用{@linkplain CommonPools#blockingTaskExecutor() the common pool}
     *
     ...
     * <br/>
     * 给默认的VirtualHost配置一个Service、通过传入的函数。
     */
     ...
     * <br/>
     * 返回一个可以支持流式挂载{@link Service}的{@link ServiceBindingBuilder}
     */
     ...
     * <br/>
     * 在默认的VirtualHost内部， 某个明确的路径下挂载一个Service
     */
    ...
    /**
     * 在默认的VirtualHost下面， 根据指定的路径模式，去挂载一个服务
     * Binds the specified {@link Service} at the specified path pattern of the default {@link VirtualHost}.
     * e.g.
     * <ul>
     ...
     * of the default {@link VirtualHost}.  在默认的VirtualHost下面，在多个Route下面分别挂载指定的ServiceWithRoutes
     * <br/>
     * TODO IMPORTANT 2019-11-21 17:09:21 有意思，一定要搞明白
     *
     ...
     * <br/>
     * 设置默认的VirtualHostBuilder的默认hostname
     */
     ...
     * <br/>
     * 配置默认的VirtualHost，通过传递进来的customizer函数
     */
     ...
     * Adds the <a href="https://en.wikipedia.org/wiki/Virtual_hosting#Name-based">name-based virtual host</a>.
     *
     * @param hostnamePattern virtual host name regular expression   虚拟主机正则表达式
     * @return {@link VirtualHostBuilder} for building the virtual host
     */
    ...
    ServerBuilder decorator(Function<T, R> decorator) {
        ...
        if (this.decorator != null) {
            // 如果不为null，说明之前有有设置过，则追加就行，第一个的输出将会作为第二个的输入。
     ...
     * 通过传入的decoratingServiceFunction来装饰所有的已经绑定的Service实例
     *
     ...
     * <br/>
     * 设置RejectedRouteHandler，其将会在尝试绑定一个已存在的Route的时候，会别拒绝。默认的系统会进行warn警告日志的打印
     */
    ...
    /**
     * Sets the timeout of a request.  请求的超时时间
     *
    ...
    /**
     * Sets the timeout of a request in milliseconds. 请求的超时时间， ms
     *
    ...
    /**
     * Sets the timeout of a request.  设置请求超时时间
     *
     * @param requestTimeout the timeout. {@code 0} disables the timeout.  0：表示禁用超时时间。
     */
    ...
    /**
     * Sets the timeout of a request in milliseconds.   设置请求超时时间 毫秒ms
     *
     * @param requestTimeoutMillis the timeout in milliseconds. {@code 0} disables the timeout. 0：表示禁用超时时间。
     */
    ...
    /**
     * Returns the timeout of a request in milliseconds.  返回请求超时时间 ms
     */
    ...
    /**
     * Sets the maximum allowed length of the content decoded at the session layer.
     * e.g. the content length of an HTTP request.   设置请求体的最大长度
     *
     * @param maxRequestLength the maximum allowed length. {@code 0} disables the length limit.  0: 表示禁用
     */
     ...
     * <br/>
     * 设置是否开启冗余响应， 开启：响应将包括异常类型、异常栈等信息； 关闭：将不会暴露服务端的细节信息。
     * 默认情况下是通过{@link Flags#verboseResponses()}来获取的。default false
     */
     ...
     * <br/>
     * 为当前Server的请求预检测工厂和响应预检测工厂，设置相同的预检测工厂
     */
     ...
     * <br/>
     * 返回到目前为止最新的Server[所有的已经配置到此时的属性，而产生的最新Server]
     */
    public Server build() {
