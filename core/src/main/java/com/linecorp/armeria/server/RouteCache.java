...
final class RouteCache {
    ...
    // 用来做缓存的容器
    // VirtualHostRouter 和 CompositeServiceRouter都会用其作为缓存容器。
    // tips: Flags.routeCacheSpec()的执行结果将会通过.map(buildCache)传递给buildCache(xxx)方法的形参
    @Nullable
     ...
     * <br/>
     * 返回被加了一层缓存的Router，加缓存是为了提高ServiceConfig的效率。
     */
     ...
     * <br/>
     * 返回被加了一层缓存的Router，加缓存是为了提高CompositeServiceEntry的效率。
     */
    ...
    private static final class CachingRouter<V> implements Router<V> {
        ...
        private final Cache<RoutingContext, V> cache; // cache == CACHE
